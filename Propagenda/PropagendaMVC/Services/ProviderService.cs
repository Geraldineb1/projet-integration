...
namespace PropagendaMVC.Services
{
    public class ProviderService : BaseHttpService, IProviderService
    {
        ...
        public async Task<ProviderWithServicesVM> GetProviderDetails(int id)
        {
            AddBearerToken();
            var provider = await _client.ProviderGETAsync(id);
            
            return _mapper.Map<ProviderWithServicesVM>(provider);
        }

        public async Task<List<ProviderWithServicesVM>> GetProviders()
        {
            AddBearerToken();
            var providers = await _client.ProviderAllAsync();
            return _mapper.Map<List<ProviderWithServicesVM>>(providers);
        }
        ...
        public async Task<List<ProviderVM>> GetProvidersByUser()
        {
            AddBearerToken();
            var providers = await _client.ProviderByUserAsync();
            return _mapper.Map<List<ProviderVM>>(providers);
        }
